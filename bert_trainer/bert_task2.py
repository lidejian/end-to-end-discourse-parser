...
def _attention(input_matrix_1, input_matrix_2):
...
def _self_attention(input_matrix):
    ...
    # 2. sum 所有的hidden state
    # (None, hidden_size)
    r = tf.reduce_sum(input_matrix, 1)

    # (None, 1, hidden_size)
    r_temp = tf.transpose(tf.expand_dims(r, -1), [0, 2, 1])
    # (None, sent_length)
    p = tf.nn.softmax(tf.reduce_sum(input_matrix * r_temp, 2))

    # (None, 1, sent_length)
    p_temp = tf.transpose(tf.expand_dims(p, -1), [0, 2, 1])
    # (None, hidden_size, sent_length)
    outputs_temp = tf.transpose(input_matrix, [0, 2, 1])
    # (None, sent_length, hidden_size)
    v = tf.reduce_sum(outputs_temp * p_temp, 2)
...
class Attention_RNN2(object):
    def __init__(self,
        ...
        with tf.name_scope("attention"):
            ...
            self.v1, self.v2 = _attention(outputs1, outputs1)
...
# my self attention neural network
class Attention_RNN3(object):
    def __init__(self,
        ...
        with tf.name_scope("attention"):
            ...
            con =  tf.concat([outputs1, outputs2],1)

            self.output = _self_attention(con)
...
class Attention_RNN4(object):
    def __init__(self,
        ...
        with tf.name_scope("attention"):
            ...
            self.output = tf.concat([self.v1, self.v2, tf.subtract(self.v1, self.v2), self.v_bi],1)
