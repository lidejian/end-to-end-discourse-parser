...
sys.path.append("../")
from bert_trainer.bert_task import RNN, Attention_RNN1, Attention_RNN2, Attention_RNN3, Attention_RNN4, \
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def train_step(s_batch, y_batch):
            """
        ...
        def test_step(s_all, y_all, test_str):
            """
        ...
        def _prediction_on_dev(best_score, best_output_string):
            ...
            confusionMatrix = test_step(dev_arg, dev_labels, test_str='dev') #得到4*4的矩阵
        ...
        def _prediction_on_test():
            ...
            confusionMatrix = test_step(test_arg, test_labels, test_str='test') #得到4*4的矩阵
            acc = confusionMatrix.get_accuracy()
            curr_output_string = confusionMatrix.get_matrix() + confusionMatrix.get_summary()
            print("\nEvaluation on test:")
            print("Current Performance:")
        ...
        batches = data_helpers.batch_iter(
            list(zip(train_arg, train_labels)), FLAGS.batch_size, FLAGS.num_epochs, shuffle=True)

        best_score = 0.0
        best_output_string = ""
        # Training loop. For each batch...
        for batch in batches:
            s_batch, y_batch = list(zip(*batch))
            train_step(s_batch, y_batch)
